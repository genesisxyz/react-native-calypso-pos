...
class ByteUtilsModule (reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext){
  ...
  fun bytesFromString(str: String, promise: Promise){
    val res = str.toByteArray(StandardCharsets.UTF_8)
    promise.resolve(ByteConvertReactNativeUtil.byteArrayToReadableArray(res))
  }
